#!/usr/bin/env python3
"""Create a deterministic one-layer Llama MoE GGUF for PadQuant mul_mat_id tests."""
...
def tensor(rng: np.random.Generator, shape: tuple[int, ...]) -> np.ndarray:
    return (rng.standard_normal(shape, dtype=np.float32) * np.float32(0.02)).astype(np.float32)
...
def main() -> None:
    ...
    n_layer = 1
    n_expert = 2
    rng = np.random.default_rng(0x50414D4F)
    ...
    writer.add_name("PadQuant deterministic toy Llama MoE")
    writer.add_context_length(128)
    writer.add_embedding_length(n_embd)
    writer.add_block_count(n_layer)
    writer.add_feed_forward_length(n_ff)
    writer.add_expert_feed_forward_length(n_ff)
    ...
    # NumPy shapes are reversed by gguf-py into GGML [ne0, ne1, ne2].
    ...
    writer.add_tensor("blk.0.ffn_gate_inp.weight", tensor(rng, (n_expert, n_embd)))
    writer.add_tensor("blk.0.ffn_gate_exps.weight", tensor(rng, (n_expert, n_ff, n_embd)))
    writer.add_tensor("blk.0.ffn_up_exps.weight", tensor(rng, (n_expert, n_ff, n_embd)))
    writer.add_tensor("blk.0.ffn_down_exps.weight", tensor(rng, (n_expert, n_embd, n_ff)))
    ...
    print(
        f"architecture=llama layers={n_layer} n_embd={n_embd} n_ff={n_ff} "
        f"experts={n_expert} vocab={n_vocab}"
    )
